...
You can find most of the files that we use for this chapter [here](https://github.com/PPBDS/all.primer.tutorials/raw/master/inst/tutorials/033-data-files/data/). We can also access these files by saving the URL to R and then using it so that we can download and access the files located on the internet.
...
But that can still cause problems down the line and it's just annoying to use backticks every time you want a column name. The `clean_names()` function from the **janitor** package formats the column names so that they follow the [underscore separated naming convention](https://en.wikipedia.org/wiki/Snake_case) and are all unique.
...
<!-- The **dbpylr** package allows you to use many of the same functions from the **tidyverse** like `select()`, `filter()`, `mutate()` without any issues. -->

<!-- Here's an example of using the **dbpylr** package to get the number of albums each artist has created. -->

```{r}
#| echo: false
# DK: This code used to work. It now fails. So, for now, I am commenting out
# this whole section. The error arises in the last select(). Why does that not
# work? Perhaps the class, which is tbl_SQLiteConnection, makes for trouble . .
# .
...
invisible(file.remove("chinook.db"))
```
...
We can do web scraping by using the **rvest** package, which is a library that allows us to look at HTML and CSS selectors, then pick them apart to get the data we want. Let's look at what this means.
...
```{r}
#| echo: false
#| fig.cap: A little bit of HTML editing can bring you a long way.
...
```{r}
#| fig.cap: You can see the HTML code of a website and even edit it temporarily by inspecting
#|   the HTML code of the in
...
{{< video https://www.youtube.com/embed/YdIWI6K64zo >}}
...
<!-- Useful? {{< video https://www.youtube.com/embed/O9HKbdhqYzk >}} -->
...
<!-- DK: Awkwardly written. Not an interesting plot. -->
