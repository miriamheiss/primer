...
<!-- Switch out unemployment data anymore. Use two or three of the temperature series from primer.data::temperature.  I have similar concerns about the `group` aesthetic, especially for line plots. But maybe group is too difficult? But can you possibly do geom_line without showing groups? -->
...
This chapter focuses on **ggplot2**, one of the core packages in the *[Tidyverse](https://www.tidyverse.org/)*, a collection of 25 or so packages which have a similar design philosophy. The **[tidyverse](https://tidyverse.tidyverse.org/)** (note the small "t") is an R package that contains the 9 key packages, most importantly **ggplot2**. In other words, there are two separate things called "tidyverse": a capital "T" *Tidyverse* which refers to many packages which work well together and a small "t" **tidyverse** which is an R package which loads the key *Tidyverse* packages. 
...
<!-- DK: Note that the <span> trick works for html. For PDF, you need something with \textcolor{}{}. See: https://bookdown.org/yihui/rmarkdown-cookbook/font-color.html -->
...
```{r}
#| echo: false
#| fig.cap: filter() reduces the rows in a tibble.

knitr::include_graphics("01-visualization/images/filter.png")
```

Please read the [filter section](https://r4ds.hadley.nz/data-transform.html#filter) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/).
...
```{r}
#| echo: false
#| fig.cap: select() reduces the number of columns in a tibble.
...
Please read the [select section](https://r4ds.hadley.nz/data-transform.html#sec-select) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/).
...
Please read the [arrange section](https://r4ds.hadley.nz/data-transform.html#arrange) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/).


 <!-- *Each step in the pipe starts with a tibble and then, once it is done, produces a tibble. It is [tibbles all the way down](https://en.wikipedia.org/wiki/Turtles_all_the_way_down)!* -->
...
```{r}
#| echo: false
#| fig.cap: '`mutate() adds a column to a tibble.'
...
Please read the [mutate section](https://r4ds.hadley.nz/data-transform.html#sec-mutate) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/).
...
<!-- DK: Other things to include here? -->
...
<!-- This whole section should be rethought. What should we highlight? What can be dropped? -->
...
There! Within the `theme()` function, we can change the plot title to bold, the background to blue, and get rid of the "minor" grid lines. `theme()` is a very [versatile function](https://ggplot2-book.org/polishing.html#polishing). S
