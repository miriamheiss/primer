...
Use `slice()`, `pull()` and `[]` to pick out specific rows or columns from a tibble.

`slice()` gives us specific rows from the `trains` tibble:
...
Unlike `filter()`, `slice()` relies on the order that the rows are currently in. 
