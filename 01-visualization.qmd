...
<!-- Rationalize other command section, coordinate with tutorial overview. Do we really need pull() and []. When do we introduce $? -->
...
`pull()` is similar to the `$` operator, which can be used outside of a pipeline and allows us to extract a single variable from a tibble and return it as a vector.
