...
<!-- Use twitter comment about transforming R code chunks to new Quarto format. -->

<!-- Case study of polls needs to be updated to use our language of validity and so on. -->


<!-- RS: Technically, standard error should not use sigma in it's formula, as it specifically refers to a point statistic and not a population. Someone may want to change all the sigmas to s. Hmm? -->

<!-- There needs to be more explanation about confidence intervals!  -->
...
<!-- Great summary! Use this exact phrasing in every chapter. -->
...
```{r}
#| echo: false
#| fig.cap: When you see another Precetor Table section.
...
```{r}
#| echo: false
tibble(source = c("Other", "Other", "Other","...",  
                  "Data", "Data", "Data", "...",
                  "Other", "Other", "Other","...", 
...
<!-- DK: Awkwardy written. Shorten. Cut? -->
...
Let's tie elements of the real life poll in this new article with our "tactile" and "virtual" urn activity from @sec-sampling-activity and @sec-virtual-sampling using the terminology, notations, and definitions we learned in @sec-standard-errors. You'll see that our sampling activity with the urn is an idealized version of what pollsters are trying to do in real life. 
...
We have a journey from reality, to our predictions, to the standard error of our predictions, to the posterior probability distribution for $\rho$. This is our sequence:
...
This journey shows how our beliefs about the truth develop through our work. We begin with $\rho$; $\rho$ is the truth, the true but unknown value we are estimating. $\hat{\rho}$ is our estimate for $\rho$. There can be millions and millions of $\hat{\rho}$'s. Next, we must take the standard error of our estimates (our $\hat{\rho}$'s) to account for our uncertainty of our predictions. Finally -- the thing we need most -- we create a posterior probability distribution for $\rho$. This distribution is used to answer key questions about $\rho$. 
