...
<!-- We do not run prep_rstudio_settings() because this function only works with an associated RStudio instance. There is no such instance when you render a qmd file, even if you render that file from within RStudio. (I think!) -->
...
1.  [Download and install R](https://cloud.r-project.org/). If you are using a Mac, make sure the use the correct installation, depending on whether you are using an  older "Intel Mac" (first option) or a newer "M1 Mac" (second option). You can look up the chip used in your Mac by checking out "About This Mac" under the Apple menu.
...
This might fail, especially if the quotation marks you paste over turn "curly." In that case, type the commands yourself from scratch.
...
```
  ...
  - Run this command to set up RStudio:
